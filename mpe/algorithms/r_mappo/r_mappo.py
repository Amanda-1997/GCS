...
class R_MAPPO():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        self.n_agents = self.args.n_agents  ### self.n_rollout_threads, num_agents, act_shape
        self.n_actions = self.args.n_actions

        epi_roll = self.episode_length * self.n_rollout_threads
        obs_batch_ = torch.tensor(obs_batch.reshape(epi_roll, self.n_agents, -1)).cuda()
        agent_id_graph_ = torch.eye(self.n_agents).unsqueeze(0).repeat(epi_roll, 1, 1).cuda()
        action_ids = np.repeat(np.expand_dims(np.eye(self.args.n_actions), axis=0), epi_roll*self.n_agents, 0)
        last_actions_batch_ =[action_ids[i][last_actions_batch.astype(np.int32)[i]] for i in range(len(action_ids))]
        last_actions_batch_ = torch.tensor(last_actions_batch_).reshape(epi_roll, self.n_agents, -1).float().cuda()
