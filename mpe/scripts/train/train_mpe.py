#!/usr/bin/env python
import sys
sys.path.append("/data/home/scv2321/MADT-deepspeed/mappo-mpe-acg-v1")
import os
# import wandb
import socket
import setproctitle
...
def parse_args(args, parser):
    ...
    all_args.n_rollout_threads = 1  # 128
    all_args.n_training_threads = 1 # 127
    all_args.episode_length = 25
    all_args.num_env_steps = 20000000
    # all_args.num_env_steps = 100
...
def main(args):
    ...
    if all_args.use_wandb:
        ...
        if not run_dir.exists():
            ...
            exst_run_nums = [int(str(folder.name).split('_')[1]) for folder in run_dir.iterdir() if str(folder.name).startswith('run')]
        ...
        curr_run = curr_run + "_" + cur_time.strftime("[%m-%d]%H.%M.%S")
